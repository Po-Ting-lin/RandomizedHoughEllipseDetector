...
class Candidate(object):
    ...
    def is_similar(self, candidate):
        ...
        if (major_axis_dist < self.info.SimilarMajorAxisDist) and (
                minor_axis_dist < self.info.SimilarMinorAxisDist) and (center_dist < self.info.SimilarCenterDist) and (
                angle_final < self.info.SimilarAngleDist):
            return True
        else:
            return False

    @staticmethod
    def __average_weight(old, now, score):
        return (old * score + now) / (score + 1)

    def __str__(self):
        return "{:6.2f}, {:6.2f}, {:6.2f}, {:6.2f}, {:6.2f}, {:6.2f}".format(self.p, self.q, self.a, self.b, self.angle, self.score)
...
class Accumulator(object):
    ...
    def get_best_candidate(self):
        self.accumulator = sorted(self.accumulator, key=lambda candidate: candidate.score, reverse=True)
    ...
    def __str__(self):
        text = ""
        print("{: <6}, {: <6}, {: <6}, {: <6}, {: <6}, {: <6}".format("p", "q", "a", "b", "angle", "score"))
        for idx, candidate in enumerate(self.accumulator):
...
class EllipseDetectorInfo(object):
    MaxIter = 1000
...
class EllipseDetector(object):
    ...
    def run(self, img, mask):
        self.__assert_img(img)
        self.__assert_img(mask)
        self.origin = img
        self.mask = mask
        ...
        if len(self.edge_pixels) > 100:
            self.info.MaxIter = len(self.edge_pixels) * 20
        print("The number of iterations: {}".format(self.info.MaxIter))
    ...
    def __find_candidate(self):
        for i in range(self.info.MaxIter):
            ...
            # find axis
            try:
                semi_major, semi_minor, angle = self.__find_semi_axis(point_package, center)
            except np.linalg.LinAlgError:  # Singular matrix
                continue
            except RHTException:
                continue
            ...
            # out of mask?
            if self.__is_ellipse_out_of_mask(center, axis, angle):
                continue
    ...
    def __find_center(self, pt):
        ...
        # assert center
        self.__point_out_of_image(center)
        return center

    def __find_semi_axis(self, pt, center):
        # shift to origin
    ...
    def __assert_diameter(self, major, minor):
    ...
    def __point_out_of_image(self, point):
    ...
    def __is_ellipse_out_of_mask(self, center, axis, angle):
    ...
    def __calculate_rotation_angle(self, a, b, c):
        if a == c:
            angle = 0
        else:
